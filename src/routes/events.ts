...
eventsRouter.use(authenticateJWT, injectAccessScope);
...
type AvStage = typeof AV_STAGES[number];

function isAvStage(value: unknown): value is AvStage {
  return typeof value === 'string' && (AV_STAGES as readonly string[]).includes(value);
}
...
eventsRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await executeTenantQuery<EventRow>(
      tenantId,
      `SELECT ${EVENT_COLS} FROM events ${whereClause} ORDER BY created_at DESC`,
      params,
    );
    ...
    const activeShows = events.filter(
      (e) => e.stage === 'confirmed_gear_reserved',
    ).length;
...
eventsRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (typeof body.project_title !== 'string' || body.project_title.trim().length === 0) {
      throw new HttpError(400, 'Field "project_title" is required');
    }
    if (typeof body.client_name !== 'string' || body.client_name.trim().length === 0) {
      throw new HttpError(400, 'Field "client_name" is required');
    }
    ...
    if (typeof body.estimated_revenue === 'number' && Number.isFinite(body.estimated_revenue)) {
      ...
      typeof body.estimated_revenue === 'string' &&
      ...
      body.stage === undefined
        ...
        : isAvStage(body.stage)
          ...
          : (() => {
              throw new HttpError(
                400,
                `Invalid "stage". Allowed values: ${AV_STAGES.join(', ')}`,
              );
            })();
